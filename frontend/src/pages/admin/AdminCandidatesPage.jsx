      ...
      {!loading && !error && rows.length > 0 && (
        <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {rows.map((row) => <tr key={row.candidateId || row._id} className="border-t border-slate-100"><td className="px-3 py-2"><Link className="font-semibold text-[#d7c08a] hover:underline" to={`/admin/candidates/${row.candidateId || row._id}`}>{row.name}</Link><div className="text-xs text-slate-500">{row.email} {row.phone ? `• ${row.phone}` : ''}</div></td><td className="px-3 py-2">{formatStageLabel(row.currentStage)}</td>{showPaymentAccess ? <td className="px-3 py-2">{formatPaymentLabel(row.paymentSummaryLabel)}</td> : null}{showDocumentAccess ? <td className="px-3 py-2">{row.documentStatusLabel}</td> : null}{showSelectionAccess ? <td className="px-3 py-2">{row.interviewSelectionStatusLabel}</td> : null}<td className="px-3 py-2">{row.nextPendingAction}</td><td className="px-3 py-2">{row.lastUpdatedAt ? new Date(row.lastUpdatedAt).toLocaleDateString('en-US') : '—'}</td></tr>)}
            </tbody>
          </table>
        </div>
      )}
